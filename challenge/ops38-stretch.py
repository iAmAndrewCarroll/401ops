...
def submit_form(form_details, url, payload):
    """
    Submit form data with a specific payload to the target URL.
    
    Args:
        form_details (dict): Details of the form to submit data to.
        url (str): The URL of the target webpage.
        payload (str): The payload to be submitted.
    ...
    for input in inputs:
        ...
        if input_name:
            data[input_name] = input_value if input_value else payload
...
def scan_sql_injection(url):
    """
    Scan a webpage for SQL injection vulnerabilities.
    ...
    for form in forms:
        ...
        if response and response.status_code == 200:
            ...
            if "sql syntax" in content.lower() or "error in your sql" in content.lower():
                vuln_info = f"[+] SQL Injection Detected on {url}\n[*] Form details:\n{form_details}"
                is_vulnerable = True
...
if __name__ == "__main__":
    while True:
        ...
        log_info, vuln_info = scan_sql_injection(url)
        if vuln_info:
            print(vuln_info)
